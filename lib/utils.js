...
exports = {
  ...
  getPureId: function(resourceId) {
    var slash = "/", pos, id;
    pos = resourceId.indexOf(slash);
    id = (pos > 0) ? resourceId.substring(pos + 1) : resourceId;
  ...
  checkIdsList: function(listOfIds) {
    /**
     * Checks the argument that should contain a list of pure Ids
     * @param {array} listOfIds
     *
     */
    var index = 0, len = listOfIds.length, id;

    for (index = 0; index < len; index++) {
      id = exports.getPureId(listOfIds[index]);
      if (!id) {
        return false;
      }
    }

...
Resource.prototype.create = function (type, origins, message,
   ...
   * @param {object} body Configuration arguments
...
Resource.prototype.delete = function (resource, queryString, retry, cb) {
  /**
   * Deletes a resource
  ...
  if (arguments.length > 0) {
    ...
    '. First parameter must be a ' + resourceId.type + ' id.');
    options = utils.optionalCUParams(arguments, message);
    options.queryString = (options.args !== {}) ? options.args : undefined;
...
Resource.prototype.clone = function (resource, body, retry, cb) {
  /**
   * Creates a resource and builds customized error and resource info
   *
   * Uses HTTP POST to send resource content.
  ...
  var self = this, options,
    ...
    sendRequest,
    reqOptions,
    message = "Failed to find a clonable resource ID";

  if (arguments.length > 0) {
    resourceId = utils.getResource(resource);
    reqOptions = utils.createRequest(resourceId.type);
    this.resource = resource;
    options = utils.optionalCUParams(arguments, message);

    if (constants.CLONABLE.indexOf(resourceId.type) > -1) {
      options.args.origin = resourceId.resource;
    } else {
      throw new Error(message);
    }
    options = utils.setRetries(options);

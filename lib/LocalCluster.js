...
function LocalCluster(resource, connection) {
  ...
  var cluster, self, fillStructure;
  this.resourceId = utils.getResource(resource);
  if ((typeof this.resourceId) === 'undefined') {
    throw new Error('Cannot build a Cluster from this resource: ' + resource);
  }
  ...
  // Loads the cluster from the cluster id or from an unfinished object
  if (NODEJS && (typeof resource) === 'string' ||
      utils.getStatus(resource).code !== constants.FINISHED) {
    cluster = new Cluster(connection);
    cluster.get(this.resourceId.resource, true,

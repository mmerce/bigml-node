...
Tree.prototype.predict = function (inputData, path, missingStrategy) {
   ...
   *
   */
  var child, predicate, finalDistribution, lastNode, distribution = [],
  ...
  if (missingStrategy === constants.PROPORTIONAL) {
    prediction = this.predictProportional(inputData, path, false);
  ...
  if (this.children && inputData.hasOwnProperty(splitField(this.children))) {
    for (child in this.children) {
      predicate = this.children[child].predicate;
...
Tree.prototype.predictProportional = function (inputData, path, missingFound) {
  /**
  ...
  if (inputData.hasOwnProperty(splitField(this.children)) ||
      missingBranch(this.children)) {
